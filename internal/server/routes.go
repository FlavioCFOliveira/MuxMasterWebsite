...
func routeInfos() []render.RouteInfo {
	const landingDesc = "Zero-dependency Go HTTP router. Radix-tree O(k), 25 ns static, 45 ns / 0 alloc one-parameter Pooled — 20 % faster than httprouter, 100 % net/http compatible."
...
func docRoutes() []route {
	...
	return []route{
		...
		{path: "/docs/max-performance", title: "Maximum performance", description: "Zero-allocation hot path with PoolRequestBundle and PoolFastParams: the lifetime contract, the failure modes, and four recipes. 45 ns / 0 B / 0 allocs on a 1-param route.", upstreamURL: ghDocs + "max-performance.md", hasMarkdown: true, contentPath: "docs/max-performance.md", cache: cacheControlDocs, section: "docs", ogType: "article"},
		...
		// Examples index plus thirteen upstream examples (v1.1.0).
		// Curated order: REST API and Maximum-performance featured first, then
		// the four other v1.1.0 examples (versioning, SSE, upload-file,
		// reverse-proxy) before the legacy auth + operational tier.
		{path: "/examples/", title: "Examples", description: "Thirteen runnable MuxMaster examples: REST API, maximum-performance opt-ins, versioning, SSE, file upload, reverse proxy, auth (Basic/JWT/OAuth2), cache, SSR, more.", upstreamURL: "https://github.com/FlavioCFOliveira/MuxMaster/tree/main/examples", hasMarkdown: false, contentPath: "", cache: cacheControlLanding, section: "examples", ogType: "article"},
		...
		{path: "/examples/max-performance", title: "Maximum-performance example", description: "Every v1.1.0 opt-in combined: PoolRequestBundle, PoolFastParams, HandleFast, UseFast, Mounted pprof, plus an in-process /bench endpoint that reports the live 2.4x speed-up.", upstreamURL: "https://github.com/FlavioCFOliveira/MuxMaster/tree/main/examples/max-performance", hasMarkdown: true, contentPath: "examples/max-performance.md", cache: cacheControlDocs, section: "examples", ogType: "article", order: 2},
		{path: "/examples/versioning", title: "Versioning example", description: "Path-based (/v1, /v2) and header-based (Accept: ...;v=N) API versioning on the same router, with nested admin groups and PoolRequestBundle for zero-alloc dispatch.", upstreamURL: "https://github.com/FlavioCFOliveira/MuxMaster/tree/main/examples/versioning", hasMarkdown: true, contentPath: "examples/versioning.md", cache: cacheControlDocs, section: "examples", ogType: "article", order: 3},
		{path: "/examples/server-sent-events", title: "Server-sent events example", description: "Pool-safe SSE streaming on MuxMaster: topic-hub fan-out, drop-on-slow-subscriber backpressure, periodic server tick, and an in-browser demo page.", upstreamURL: "https://github.com/FlavioCFOliveira/MuxMaster/tree/main/examples/server-sent-events", hasMarkdown: true, contentPath: "examples/server-sent-events.md", cache: cacheControlDocs, section: "examples", ogType: "article", order: 4},
		{path: "/examples/upload-file", title: "Upload-file example", description: "Multipart upload with PoolRequestBundle: single + multi + async handlers, the body-drain-before-spawn pattern, 32 MiB caps, path-traversal guard, streaming SHA-256.", upstreamURL: "https://github.com/FlavioCFOliveira/MuxMaster/tree/main/examples/upload-file", hasMarkdown: true, contentPath: "examples/upload-file.md", cache: cacheControlDocs, section: "examples", ogType: "article", order: 5},
		{path: "/examples/reverse-proxy", title: "Reverse-proxy example", description: "HTTP gateway with httputil.ReverseProxy on MuxMaster: catch-all routing, lock-free round-robin load balancing, per-route gating, and pool-safe synchronous dispatch.", upstreamURL: "https://github.com/FlavioCFOliveira/MuxMaster/tree/main/examples/reverse-proxy", hasMarkdown: true, contentPath: "examples/reverse-proxy.md", cache: cacheControlDocs, section: "examples", ogType: "article", order: 6},
		{path: "/examples/authn", title: "Authentication example", description: "HTTP Basic Authentication via the BasicAuth middleware, paired with ThrottlePerIP to defend against credential-stuffing attacks.", upstreamURL: "https://github.com/FlavioCFOliveira/MuxMaster/tree/main/examples/authn", hasMarkdown: true, contentPath: "examples/authn.md", cache: cacheControlDocs, section: "examples", ogType: "article", order: 7},
		{path: "/examples/jwt", title: "JWT example", description: "Bearer-token authentication via the JWTAuth middleware. Configures RequireExpiry: true (RFC 8725 §4.4) and shows the canonical OIDC integration shape.", upstreamURL: "https://github.com/FlavioCFOliveira/MuxMaster/tree/main/examples/jwt", hasMarkdown: true, contentPath: "examples/jwt.md", cache: cacheControlDocs, section: "examples", ogType: "article", order: 8},
		{path: "/examples/oauth2", title: "OAuth2 example", description: "OAuth 2.0 token introspection (RFC 7662) via the OAuth2Introspect middleware. Tokens are validated against an authorisation server, not locally.", upstreamURL: "https://github.com/FlavioCFOliveira/MuxMaster/tree/main/examples/oauth2", hasMarkdown: true, contentPath: "examples/oauth2.md", cache: cacheControlDocs, section: "examples", ogType: "article", order: 9},
		{path: "/examples/cache", title: "Cache example", description: "An in-memory TTL cache that avoids re-computing identical responses within a configurable horizon. For expensive, idempotent handlers.", upstreamURL: "https://github.com/FlavioCFOliveira/MuxMaster/tree/main/examples/cache", hasMarkdown: true, contentPath: "examples/cache.md", cache: cacheControlDocs, section: "examples", ogType: "article", order: 10},
		{path: "/examples/graceful-shutdown", title: "Graceful shutdown example", description: "Production graceful shutdown: signal handling, srv.Shutdown with a bounded drain deadline, and the recommended Server timeout set for real deployments.", upstreamURL: "https://github.com/FlavioCFOliveira/MuxMaster/tree/main/examples/graceful-shutdown", hasMarkdown: true, contentPath: "examples/graceful-shutdown.md", cache: cacheControlDocs, section: "examples", ogType: "article", order: 11},
